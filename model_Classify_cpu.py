...
parser.add_argument("--input_code", type=str, default="data/sample_input.cpp")
...
with open(args.input_code, "r") as inFile:
    inputCode = inFile.read().strip()

dropout.train(False)
author_scores = runModel([inputCode])

# Turn the scores into probabilities using softmax
author_scores = torch.nn.Softmax(dim=1)(author_scores)
probabilities, predictions = author_scores.topk(k=10)
probabilities = probabilities.detach().cpu().view(-1).numpy().tolist()
predictions = predictions.detach().cpu().view(-1).numpy().tolist()
print("Top-10 Authors:")
print("\tAuthor\tProbability")
for i in range(10):
     print(i+1, "\t", predictions[i], "\t", round(100*probabilities[i],2), "%")
